...
final class NetworkServiceTests: XCTestCase {
    ...
    let sesssionData = NetworkSessionDataMock()
    override func setUpWithError() throws {
        try super.setUpWithError()
        sut = NetworkService(configuration: NetworkAPIConfigurableMock(), sessionData: sesssionData)
    }
    ...
    func test_request_whenBadURL_shouldBeFailAndNetworkErrorIsBadURL() async throws {
        ///given
        sesssionData.error = NSError(domain: "network", code: NSURLErrorBadURL)
        ///when
        do {
            _ = try await sut.request(endpoint: EndpointMock())
        } catch let error {
    ...
    func test_request_whenTimeOut_shouldBeFailAndNetworkErrorIsTimeOut() async throws {
        ///given
        sesssionData.error = NSError(domain: "network", code: NSURLErrorTimedOut)
        ///when
        do {
            _ = try await sut.request(endpoint: EndpointMock())
        } catch let error {
    ...
    //TODO: - How get error with response like 404
}
